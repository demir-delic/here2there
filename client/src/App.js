...
function App() {
  ...
  const getLocation = async () => {
    if (navigator.geolocation) {
      ...
      await axios
        .get(`https://api.ipgeolocation.io/ipgeo?apiKey=${ipKey}&ip=${ip}`)
        .then((res) => res.data)
        .then((data) => {
          setCoords({
            lat: data.latitude,
      ...
      <Switch>
        ...
        <Route path="/">
          <div className="h-min-screen bg-gray-50">
            ...
            <LandingPageHeader userCity={userCity} userCountry={userCountry} />
            <div className="container flex flex-col items-baseline justify-between w-max h-88 mx-auto px-10">
              ...
              <Switcher label={`Close to ${userCity}`} isEnabled={false} />
